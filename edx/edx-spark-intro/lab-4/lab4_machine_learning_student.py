...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    averageRating = (sum([r for r in IDandRatingsTuple[1]]) * 1.0) / len(IDandRatingsTuple[1])
...
movieIDsWithRatingsRDD = (ratingsRDD
                          .map(lambda rating: (rating[1], rating[2])).groupByKey())
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              .join(movieIDsWithAvgRatingsRDD).map(lambda (x,y): (y[1][1], y[0], y[1][0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda rec: math.pow((rec[1][0] - rec[1][1]),2))
                       )
...
validationForPredictRDD = validationRDD.map(lambda rec: (rec[0], rec[1]))
...
myModel = ALS.train(trainingRDD, 8, seed=5L, iterations=5,
                      lambda_=0.1)
testForPredictingRDD = testRDD.map(lambda rec: (rec[0], rec[1]))
...
trainingAvgRating = trainingRDD.map(lambda rec: rec[2]).sum() / trainingRDD.count()
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda rec: any(row[1] != rec[0] for row in myRatedMovies))
                      .map(lambda rec: (0, rec[0])))
...
predictedRDD = predictedRatingsRDD.map(lambda rec: (rec[1], rec[2]))
