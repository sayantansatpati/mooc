...
def simpleTokenize(string):
    ...
    """
    return [t for t in re.split(split_regex, string.lower()) if t != '']
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(15, lambda (k,v): -len(v))
...
def dotprod(a, b):
    ...
    """
    return sum([a[k] * b[k] for k in a.iterkeys() if k in b])
...
avgSimDups = trueDupsRDD.map(lambda (x,y): y[0]).sum() / trueDupsCount

nonDupsRDD = (sims
              .subtractByKey(goldStandard))
avgSimNon = nonDupsRDD.map(lambda (x,y): y).sum() / nonDupsRDD.count()
...
def invert(record):
    ...
    """
    pairs = [(k, record[0]) for k in record[1].iterkeys()]
    return (pairs)

amazonInvPairsRDD = (amazonWeightsRDD
                    .flatMap(lambda (x,y): invert((x,y)))
                    .cache())
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = (sum([amazonWeightsBroadcast.value[t] * googleWeightsBroadcast.value[t] for t in tokens
             if t in amazonWeightsBroadcast.value and t in googleWeightsBroadcast.value]))
    value = s / (amazonNormsBroadcast.value[amazonRec] * googleNormsBroadcast.value[googleRec])
    key = (amazonRec, googleRec)
    return (key, value)

similaritiesFullRDD = (commonTokens
                       .map(lambda(x,y): fastCosineSimilarity((x,y)))
                       .cache())
