...
def parsePoint(line):
    ...
    """
    tokens = line.split(",")
    return LabeledPoint(tokens[0], tokens[1:])

parsedSamplePoints = rawData.map(lambda line: parsePoint(line))
...
def twoWayInteractions(lp):
    ...
    """
    result = lp.features
    for t in itertools.product(lp.features, lp.features):
        if len(t) == 1:
            result = np.hstack((result, t[0] ** 2))
        if len(t) == 2:
            result = np.hstack((result, t[0] * t[1]))
    return LabeledPoint(lp.label, result)
